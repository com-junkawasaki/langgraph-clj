(ns langgraph.db
  ...
  Implements the subset of the Datomic API that langgraph-clj needs for
  ...
  Design constraints (see ADR-0001):
    ...
    - the API shape matches Datomic/DataScript closely enough that a
      ...
      the `langgraph.db/api` map (see bottom of this namespace).
...
(defn- expand-entity-map
  ...
  [db m counter]
  (let [e (or (:db/id m) (str "langgraph.db/auto-" (vswap! counter inc)))
        avs (->> (dissoc m :db/id)
...
(def api
  "Datomic-shaped function map. Higher layers (langgraph.checkpoint,
  langgraph.memory) take this map as their `:db-api` option, so a real
